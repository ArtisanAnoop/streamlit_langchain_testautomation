...
def create_prompt(num_accessibility, num_functional, num_performance, num_security, num_usability, num_visual):
    ...
    main_prompt = f"""
            ...
            When creating security test cases, it's essential to cover authentication and authorization mechanisms, input validation to prevent injection and XSS attacks, and data encryption both at rest and in transit. Additional focus areas include secure session management, error handling to prevent information leakage, and configuration management to avoid security misconfigurations. Finally, ensure thorough testing of file handling, third-party integrations, business logic vulnerabilities, and resilience against denial of service attacks. This comprehensive approach helps identify and mitigate potential security threats effectively.
            When creating performance test cases , consider that Performance testing is essential to evaluate how a web application behaves under various conditions of load and stress. It involves testing for speed, scalability, and stability to ensure that the application meets the performance criteria without compromising on functionality or user experience. Key aspects include load testing to simulate real-life user loads, stress testing to determine the limits of system capacity, and endurance testing to assess performance over an extended period. This type of testing helps identify bottlenecks and optimize system performance, ensuring that the application can handle its intended user traffic and activity smoothly.
            When creating visual test cases , consider that Visual testing involves assessing a web application's graphical user interface (GUI) to ensure it appears correctly across different devices and browsers. This type of testing checks for visual consistency and alignment, color correctness, and responsiveness of the layout. It uses tools that capture screenshots of various UI states and compares them against a baseline to identify discrepancies. Visual testing helps in catching unintended visual modifications that might occur due to code changes, ensuring the user interface is visually appealing and functions as designed across all platforms.
